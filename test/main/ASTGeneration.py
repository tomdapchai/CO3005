...
class ASTGeneration(MiniGoVisitor):
    ...
    def visitDecl_or_stmt(self, ctx:MiniGoParser.Decl_or_stmtContext):
        ...
        # Base case: statement (stmt_primary)
        elif ctx.stmt_primary():
            nodes.append(self.visit(ctx.stmt_primary()))
        
        # Testing part 1 task 3: expression
        elif ctx.expr():
            nodes.append(self.visit(ctx.expr()))
    ...
    def visitField_access(self, ctx:MiniGoParser.Field_accessContext):
        if ctx.getChildCount() == 1:
            ...
            if ctx.function_call():
                ...
                return CallExpr(self.visit(ctx.field_access()), function_call.method, function_call.param)
            elif ctx.ID():
                return FieldAccess(obj=self.visit(ctx.field_access()), fieldname=Id(ctx.ID().getText()))
            else:
                arr = self.visit(ctx.array_access())
                res = self.visit(ctx.field_access())
                if isinstance(arr.arr, CallExpr):
                    arr.arr.obj = res
                    return ArrayCell(arr.arr, arr.idx)
                return ArrayCell(FieldAccess(obj=self.visit(ctx.field_access()), fieldname=Id(getArrName(arr))), arr.idx)
    ...
    def visitAtom_arr_access(self, ctx:MiniGoParser.Atom_arr_accessContext):
        if ctx.getChildCount() == 1:
            ...
            return ArrayCell(self.visit(ctx.atom_arr_access()), self.visit(ctx.index_expr()))
    ...
    def visitArray_access_tail(self, ctx: MiniGoParser.Array_access_tailContext):
        ...
        if not ctx.index_expr():
            return [] # if there is no index_expr return empty list
    ...
    def visitMultiplicative_index_expr(self, ctx: MiniGoParser.Multiplicative_index_exprContext):
        if ctx.multiplicative_index_expr():
            ...
            if ctx.MUL():
                ...
                op = ctx.MOD().getText()
            right = self.visit(ctx.primary_index_expr())
            return BinaryOp(op, left, right)
        
        return self.visit(ctx.primary_index_expr())
    ...
    def visitPrimary_index_expr(self, ctx: MiniGoParser.Primary_index_exprContext):
        ...
        if ctx.secondary_index_expr():
            ...
            if ctx.signed_tail():
                ...
                expr = self.visit(ctx.int_number())
                for op in reversed(operator):
                    expr = UnaryOp(op, expr)
                return expr
            return self.visit(ctx.int_number())
    ...
    def visitArray_literal(self, ctx: MiniGoParser.Array_literalContext):
        """Visit array literal."""
        typ = self.visit(ctx.types())  # Visit the type of the array
        dimensions = self.visit(ctx.array_literal_tail3())  # Visit the list of dimensions
        values = self.visit(ctx.array_literal_tail())  # Visit the list of values
    ...
    def visitStruct_literal(self, ctx: MiniGoParser.Struct_literalContext):
        ...
        return StructLiteral(struct_name, field_inits)
    ...
    def visitStruct_field_access(self, ctx: MiniGoParser.Struct_field_accessContext):
        if ctx.getChildCount() == 1:
            ...
            if ctx.function_call():
                function_call = self.visit(ctx.function_call())
                return CallExpr(self.visit(ctx.struct_field_access()), function_call.method, function_call.param)
            elif ctx.ID():
